...
class TC:
  ...
  def volts_to_tempC(self, voltage, junctionTemp, type = None) -> float:
    offsetV = self.tempC_to_volts(junctionTemp, type)
    return self.volts_to_tempC_no_comp(voltage + offsetV, type)
    #return 1.00

  def volts_to_tempC_no_comp(self, voltage, type = None) -> float:
  ...
  def tempC_to_volts(self, temperature, type = None) -> float:
